...
public:
        ...
        uint64_t height) 
    {
        ...
        if ((pos = result.find("{COUNT}")) != std::string::npos) {
            result.replace(pos, 7, std::to_string(index + 1));
        }
        ...
        if ((pos = result.find("{BASENAME}")) != std::string::npos) {
            result.replace(pos, 10, inputBasename);
        }

        if ((pos = result.find("{EXT}")) != std::string::npos) {
            result.replace(pos, 5, inputExtension);
        }
